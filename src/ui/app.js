...
function renderAdminSummary(){
  ...
  const data=filteredData(),intel=buildAdminIntelligence({...data,taxonomy:S.taxonomy,language:S.lang,minDirectionalSample:analysisRules().minDirectionalSample}),dwell=dwellStats(data.calls),issueRows=Object.entries(intel.reasonCounts).sort((a,b)=>b[1]-a[1]);
  return `<div class="admin-page-head"><div><span class="eyebrow">ADMIN INTELLIGENCE</span><h1>Summary Join Visit</h1><p>Executive snapshot untuk execution, recovery, dan evidence quality.</p></div><div class="header-actions"><button class="btn btn-primary" data-action="exportAdmin">Export Detail</button><button class="btn btn-secondary" data-action="refresh">↻ Refresh</button></div></div><div class="card admin-filter-card">${adminFilterBar()}<button class="btn btn-ghost clear-filter-btn" data-action="clearAdminFilters">Reset Filter</button></div><div class="admin-summary-grid"><section class="summary-group"><div class="summary-group-head"><span>FIELD EXECUTION</span></div><div class="kpis"><div class="kpi featured"><b>${intel.visitEcsc}%</b><span>Visit EC/SC</span></div><div class="kpi"><b>${intel.visitSc}</b><span>Visit SC</span></div><div class="kpi"><b>${intel.visitEc}</b><span>Visit EC</span></div><div class="kpi"><b>${fmtCurrency(intel.visitRevenue)}</b><span>Visit Omzet</span></div></div></section><section class="summary-group"><div class="summary-group-head"><span>REMOTE & RECOVERY</span></div><div class="kpis"><div class="kpi"><b>${intel.waEc}</b><span>Pure WA EC</span></div><div class="kpi"><b>${fmtCurrency(intel.waRevenue)}</b><span>WA Omzet</span></div><div class="kpi"><b>${intel.recoveredEc}</b><span>Recovered EC</span></div><div class="kpi"><b>${fmtCurrency(intel.recoveredRevenue)}</b><span>Recovered Omzet</span></div></div></section><section class="summary-group"><div class="summary-group-head"><span>NON-EC & EVIDENCE</span></div><div class="kpis"><div class="kpi"><b>${intel.visitNonEc}</b><span>Non-EC Visit</span></div><div class="kpi"><b>${intel.recoverableCount}</b><span>Recoverable Today</span></div><div class="kpi"><b>${fmtDuration(dwell.avg)}</b><span>Avg Duration</span></div><div class="kpi"><b>${data.photos.length}</b><span>Photo Evidence</span></div></div></section></div><div class="admin-two-grid"><div class="card"><div class="section-head"><div><h2>Top Actual Non-EC Issues</h2><div class="meta">Root cause berdasarkan observasi JOVIS</div></div><span class="pill">N=${intel.visitNonEc}</span></div>${issueRows.map(([k,n],i)=>`<div class="issue-row ranked"><span class="rank">${i+1}</span><b>${esc(observedLabel({observed_reason_code:k},S.taxonomy,S.lang))}</b><span>${n} · ${pct(n,intel.visitNonEc)}%</span></div>`).join('')||`<div class="empty">Belum ada Non-EC.</div>`}</div><div class="card"><div class="section-head"><div><h2>Data Coverage</h2><div class="meta">Cek apakah evidence cukup untuk analisis</div></div></div><div class="coverage-list"><div><span>Visit dengan GPS</span><b>${data.calls.filter(c=>callMethod(c)==='VISIT'&&c.checkin_latitude!=null).length}/${data.calls.filter(c=>callMethod(c)==='VISIT').length}</b></div><div><span>Rich Non-EC detail</span><b>${data.reasonDetails.length}/${intel.visitNonEc}</b></div><div><span>Photo evidence</span><b>${data.photos.length}</b></div><div><span>Recovery attempts</span><b>${data.recoveryAttempts.length}</b></div></div></div></div>${renderSfaRecovery(data)}`;
}
...
function renderAdminMap(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData(),gpsCount=data.calls.filter(c=>callMethod(c)==='VISIT'&&c.checkin_latitude!=null).length;return `<div class="admin-page-head compact"><div><span class="eyebrow">JOIN VISIT MAP</span><h1>Perjalanan JOVIS</h1><p>Marker mengikuti urutan check-in toko; BY WA tidak mempunyai titik GPS.</p></div><button class="btn btn-secondary" data-action="refresh">↻ Refresh</button></div><div class="card admin-filter-card">${adminFilterBar()}<div class="map-filter-row"><select class="select" data-map-filter="result"><option value="">Semua Result</option><option value="EC" ${S.mapFilters.result==='EC'?'selected':''}>EC</option><option value="NON_EC" ${S.mapFilters.result==='NON_EC'?'selected':''}>NON_EC</option></select><select class="select" data-map-filter="reason"><option value="">Semua Reason</option>${S.taxonomy.filter(x=>x.reason_type==='observed').map(x=>`<option value="${x.reason_code}" ${S.mapFilters.reason===x.reason_code?'selected':''}>${esc(x.reason_label_id)}</option>`).join('')}</select></div></div><div class="map-statbar"><span><b>${gpsCount}</b> GPS calls</span><span><b>${data.visits.length}</b> visits</span><span><b>${data.calls.filter(c=>c.result==='NON_EC').length}</b> Non-EC</span></div><div class="card map-card"><div id="adminMap" class="admin-map"></div><div class="meta map-note">Garis menunjukkan urutan check-in, bukan actual road travelled.</div></div>`}
...
function renderAdminAnalysis(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData(),intel=buildAdminIntelligence({...data,taxonomy:S.taxonomy,language:S.lang,minDirectionalSample:analysisRules().minDirectionalSample});return `<div class="admin-page-head compact"><div><span class="eyebrow">RULE-BASED RESEARCH</span><h1>Pertanyaan Join Visit</h1><p>Kesimpulan hanya keluar jika field dan sample mendukung; selain itu ditandai evidence insufficient.</p></div></div><div class="card admin-filter-card">${adminFilterBar()}</div><div class="research-grid">${intel.questions.map(q=>`<div class="card research-card"><div class="research-top"><span class="pill ${q.status==='DIRECTIONAL'?'good':'warn'}">${q.status==='DIRECTIONAL'?'Directional':'Evidence Insufficient'}</span><span class="sample-badge">N=${q.sample}</span></div><div class="question-id">${q.id}</div><h2>${esc(q.title)}</h2><p>${esc(q.answer)}</p></div>`).join('')}</div><div class="admin-two-grid"><div class="card"><h2>Recovery Channel</h2><div class="channel-list">${Object.entries(intel.channels).map(([k,x])=>`<div><span>${esc(k)}</span><b>${x.recovered}/${x.attempts}</b><small>${x.conversion}% · ${fmtCurrency(x.revenue)}</small></div>`).join('')||'<div class="empty">Belum ada recovery attempt.</div>'}</div></div><div class="card"><h2>BMA · Pengakuan Salesman</h2><div class="kpis"><div class="kpi"><b>${intel.bombing.YES||0}</b><span>Terlalu besar: Ya</span></div><div class="kpi"><b>${intel.bombing.NO||0}</b><span>Tidak</span></div><div class="kpi"><b>${intel.bombing.UNKNOWN||0}</b><span>Tidak tahu</span></div><div class="kpi"><b>${intel.bombing.UNANSWERED||0}</b><span>Belum dicapture</span></div></div><div class="notice" style="margin-top:10px">Claim salesman bukan bukti bombing. Validasi final tetap menggunakan DBase historical order.</div></div></div>`}
