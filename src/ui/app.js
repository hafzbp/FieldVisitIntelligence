...
const S={session:null,user:null,profile:null,profiles:[],visits:[],calls:[],taxonomy:[],reasonMappings:[],lang:'id',route:'home',selectedVisitId:null,callDraft:null,callStage:0,editingCallId:null,geoBusy:false,geoError:'',sync:{online:navigator.onLine,pending:0,errors:0,syncing:false},queueItems:[],diagnostics:null,filters:{date:'',jovis:'',depot:''},adminTimer:null};
...
async function refreshData({quiet=false}={}){
  if(navigator.onLine){
    ...
    try{
      ...
      const cloudVisits=data.visits.filter(v=>!pendingVisit.has(v.id));const cloudCalls=data.calls.filter(c=>!pendingCall.has(c.id));
...
function route(r,payload={}){if(S.adminTimer){clearInterval(S.adminTimer);S.adminTimer=null}S.route=r;if(payload.visitId)S.selectedVisitId=payload.visitId;if(r==='field'&&!S.callDraft)resetCallDraft();render();if(r==='settings')loadDiagnosticsData().then(()=>{if(S.route==='settings')render()}).catch(()=>{});if(r==='admin'&&FEATURE_FLAGS.adminAutoRefresh&&isAdmin())S.adminTimer=setInterval(()=>refreshData({quiet:false}),FEATURE_FLAGS.adminAutoRefreshSeconds*1000)}
function render(){
  if(!S.user){renderLogin();return}
  let content='';
