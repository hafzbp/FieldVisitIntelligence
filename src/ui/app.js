...
const S={
  ...
  lang:'id',route:'home',selectedVisitId:null,selectedRecoveryCallId:null,adminSelectedCallId:null,
...
function navIcon(name){const icons={home:'⌂',field:'＋',analysis:'▥',summary:'▦',detail:'≣',map:'⌖',settings:'⚙'};return `<span class="nav-glyph">${icons[name]||'•'}</span>`}
function bottomNav(){
  const items=isAdmin()?[
...
function renderHome(){
  const av=activeVisit(),vis=[...myVisits()].sort((a,b)=>String(b.start_time).localeCompare(String(a.start_time))).slice(0,10),pending=pendingRecoveryCalls();
  return `<div class="card hero"><span class="pill good">${t('roleJovis')} · ${esc(S.profile.display_name||S.user.email)}</span><h1 style="margin-top:10px">Field Visit Hari Ini</h1><p class="muted">Call VISIT menggunakan GPS; pure order WhatsApp dicatat terpisah sebagai BY WA.</p>${av?`<button class="btn btn-primary btn-block" data-route="field">${t('resume')} · ${visitStats(av).sc} Visit SC</button>`:`<button class="btn btn-primary btn-block" data-route="setup">+ ${t('startVisit')}</button>`}</div>
  ${pending.length?`<div class="card"><div class="section-head"><h2>Recovery Pending</h2><span class="pill warn">${pending.length}</span></div>${pending.map(c=>`<div class="reason-card"><b>${esc(c.outlet_id||'')} · ${esc(c.outlet_name)}</b><div class="meta">${esc(observedLabel(c,S.taxonomy,S.lang))} · ${fmtTime(c.checkin_at||c.call_timestamp)}</div><button class="btn btn-secondary" style="margin-top:8px" data-recovery-call="${c.id}">+ Update Recovery</button></div>`).join('')}</div>`:''}
  <div class="card"><div class="section-head"><h2>${t('myVisits')}</h2><span class="small muted">${vis.length}</span></div>${vis.length?`<div class="list">${vis.map(visitCard).join('')}</div>`:`<div class="empty">${t('noData')}</div>`}</div>`;
}
function visitCard(v){const z=visitStats(v);return `<div class="list-card"><div class="list-top"><div><div class="list-title">${esc(v.depot)} · ${esc(v.salesman_name)}</div><div class="meta">${fmtDate(v.visit_date)} · ${fmtTime(v.start_time)}</div></div><span class="pill ${v.status==='active'?'blue':'good'}">${v.status==='active'?t('active'):t('completed')}</span></div><div class="metrics"><span class="metric">Visit SC ${z.sc}</span><span class="metric">EC ${z.ec}</span><span class="metric">EC/SC ${z.ecsc}%</span><span class="metric">WA ${z.wa}</span></div><div class="btn-row" style="margin-top:9px"><button class="btn btn-secondary" data-open-visit="${v.id}">${t('analysis')}</button><button class="btn btn-secondary" data-edit-visit="${v.id}">${t('edit')}</button></div></div>`}
...
function renderField(){
  ...
  if(!S.callDraft)resetCallDraft();const cs=visitCalls(v.id),z=visitStats(v);
  let body='';
  ...
  return `<div class="two-col"><section><div class="card"><div class="call-header"><div class="num">${S.editingCallId?'✎':cs.length+1}</div><div class="info"><h2 style="margin:0">${esc(v.depot)} · ${esc(v.salesman_name)}</h2><div class="meta">Visit SC ${z.sc} · EC ${z.ec} · EC/SC ${z.ecsc}% · WA ${z.wa}</div></div></div>${body}</div></section><aside><div class="card"><div class="section-head"><h2>Call Terakhir</h2><span class="pill blue">${cs.length}</span></div>${cs.slice(-8).reverse().map(c=>`<div class="reason-card"><div class="list-top"><div><b>${esc(c.outlet_id||'')} · ${esc(c.outlet_name)}</b><div class="meta">${callMethod(c)==='WHATSAPP'?'BY WA':fmtTime(c.checkin_at||c.call_timestamp)} · ${c.result}${c.result==='EC'?` · ${fmtCurrency(c.omzet)}`:` · ${esc(observedLabel(c,S.taxonomy,S.lang))}`}</div></div><button class="btn btn-secondary" data-edit-call="${c.id}" data-visit-id="${v.id}">Edit</button></div>${c.result==='NON_EC'?`<button class="btn btn-soft" style="margin-top:7px" data-recovery-call="${c.id}">+ Update Recovery</button>`:''}</div>`).join('')||`<div class="empty">${t('noData')}</div>`}<hr><button class="btn btn-danger btn-block" data-action="endVisit">${t('endVisit')}</button></div></aside></div>`;
}
...
function captureInputs(){
  ...
  document.querySelectorAll('[data-stock-index][data-stock-field]').forEach(el=>{const i=Number(el.dataset.stockIndex);if(S.stockDraft[i])S.stockDraft[i][el.dataset.stockField]=el.value});
...
async function persistCallChildren(call){
  if(call.result==='NON_EC'){
    const detail={...(S.callDetailDraft||emptyReasonDetail(call.id,S.user.id)),call_id:call.id,jovis_user_id:call.jovis_user_id,source_version:'0.4.0'};
...
function renderAdminSummary(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData(),intel=buildAdminIntelligence({...data,taxonomy:S.taxonomy,language:S.lang,minDirectionalSample:analysisRules().minDirectionalSample}),dwell=dwellStats(data.calls);return `<div class="card hero"><div class="section-head"><div><span class="pill blue">ADMIN INTELLIGENCE</span><h1 style="margin-top:8px">Summary Join Visit</h1><p class="muted small">Admin tidak memiliki workflow Visit. Fokus: summary, detail, map, analysis, settings.</p></div><div class="header-actions"><button class="btn btn-primary" data-action="exportAdmin">Export Detail</button><button class="btn btn-secondary" data-action="refresh">↻ Refresh</button></div></div>${adminFilterBar()}<div class="kpis"><div class="kpi"><b>${intel.visitSc}</b><span>Visit SC</span></div><div class="kpi"><b>${intel.visitEc}</b><span>Visit EC</span></div><div class="kpi"><b>${intel.visitEcsc}%</b><span>Visit EC/SC</span></div><div class="kpi"><b>${fmtCurrency(intel.visitRevenue)}</b><span>Visit Omzet</span></div><div class="kpi"><b>${intel.waEc}</b><span>Pure WA EC</span></div><div class="kpi"><b>${fmtCurrency(intel.waRevenue)}</b><span>WA Omzet</span></div><div class="kpi"><b>${intel.recoveredEc}</b><span>Recovered EC</span></div><div class="kpi"><b>${fmtCurrency(intel.recoveredRevenue)}</b><span>Recovered Omzet</span></div><div class="kpi"><b>${intel.visitNonEc}</b><span>Non-EC Visit</span></div><div class="kpi"><b>${intel.recoverableCount}</b><span>Recoverable Today</span></div><div class="kpi"><b>${fmtDuration(dwell.avg)}</b><span>Avg Visit Duration</span></div><div class="kpi"><b>${data.photos.length}</b><span>Photo Evidence</span></div></div></div>${renderSfaRecovery(data)}<div class="card"><h2>Top Actual Non-EC Issues</h2>${Object.entries(intel.reasonCounts).sort((a,b)=>b[1]-a[1]).map(([k,n])=>`<div class="issue-row"><b>${esc(observedLabel({observed_reason_code:k},S.taxonomy,S.lang))}</b><span>${n} case · ${pct(n,intel.visitNonEc)}%</span></div>`).join('')||`<div class="empty">Belum ada Non-EC.</div>`}</div>`}
...
function renderAdminDetail(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData(),rows=adminRows(data),selected=S.adminSelectedCallId?rows.find(r=>r.call.id===S.adminSelectedCallId):null;return `<div class="card"><div class="section-head"><div><span class="pill blue">ROW-LEVEL EXPLORER</span><h1 style="margin-top:8px">Detail Call</h1></div><button class="btn btn-primary" data-action="exportAdmin">Export</button></div>${adminFilterBar()}<div class="meta">${rows.length} row sesuai filter. Setiap header punya filter sendiri.</div></div>${selected?renderSelectedCall(selected,data):''}<div class="card admin-table-card"><div class="table-wrap"><table class="table admin-detail-table"><thead><tr><th>Date</th><th>JOVIS</th><th>Depot</th><th>Salesman</th><th>Kode Toko</th><th>Nama Toko</th><th>Method</th><th>Route</th><th>Result</th><th>Omzet</th><th>Actual</th><th>SFA Exact</th><th>Recovery</th><th>Duration</th><th>Lat/Long</th><th>Evidence</th><th>Photo</th><th></th></tr><tr class="filter-row"><th>${detailFilterInput('date')}</th><th>${detailFilterInput('jovis')}</th><th>${detailFilterInput('depot')}</th><th>${detailFilterInput('salesman')}</th><th>${detailFilterInput('outletId')}</th><th>${detailFilterInput('outletName')}</th><th>${detailFilterInput('method','select')}</th><th>${detailFilterInput('route','select')}</th><th>${detailFilterInput('result','select')}</th><th>${detailFilterInput('omzet')}</th><th>${detailFilterInput('actual')}</th><th>${detailFilterInput('sfa')}</th><th>${detailFilterInput('recovery','select')}</th><th>${detailFilterInput('duration')}</th><th>${detailFilterInput('gps')}</th><th>${detailFilterInput('evidence')}</th><th>${detailFilterInput('photo','select')}</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td>${esc(r.date)}</td><td>${esc(r.jovis)}</td><td>${esc(r.depot)}</td><td>${esc(r.salesman)}</td><td><b>${esc(r.outletId)}</b></td><td>${esc(r.outletName)}</td><td>${esc(r.method)}</td><td>${esc(r.route)}</td><td>${esc(r.result)}</td><td>${r.result==='EC'?fmtCurrency(r.omzet):''}</td><td>${esc(r.actual)}</td><td>${esc(r.sfa)}</td><td>${esc(r.recovery)}</td><td>${fmtDuration(r.duration)}</td><td>${r.lat!==''&&r.long!==''?`<a class="coord-link" target="_blank" rel="noopener" href="https://www.google.com/maps?q=${encodeURIComponent(`${r.lat},${r.long}`)}">${Number(r.lat).toFixed(5)}, ${Number(r.long).toFixed(5)}</a><div class="tiny muted">±${Math.round(Number(r.accuracy)||0)}m</div>`:'-'}</td><td class="evidence-cell">${esc(r.evidence)}</td><td>${r.photoCount?`<button class="btn btn-secondary" data-photo-call="${r.call.id}">📷 ${r.photoCount}</button>`:'-'}</td><td><div class="action-stack"><button class="btn btn-secondary" data-show-call="${r.call.id}">Detail</button><button class="btn btn-secondary" data-admin-edit-call="${r.call.id}" data-visit-id="${r.call.visit_id}">Edit</button></div></td></tr>`).join('')}</tbody></table></div></div>`}
...
function renderAdminMap(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData();return `<div class="card"><div class="section-head"><div><span class="pill blue">JOIN VISIT MAP</span><h1 style="margin-top:8px">Map per JOVIS / Salesman</h1></div><button class="btn btn-secondary" data-action="refresh">↻ Refresh</button></div>${adminFilterBar()}<div class="map-filter-row"><select class="select" data-map-filter="result"><option value="">Semua Result</option><option value="EC" ${S.mapFilters.result==='EC'?'selected':''}>EC</option><option value="NON_EC" ${S.mapFilters.result==='NON_EC'?'selected':''}>NON_EC</option></select><select class="select" data-map-filter="reason"><option value="">Semua Reason</option>${S.taxonomy.filter(x=>x.reason_type==='observed').map(x=>`<option value="${x.reason_code}" ${S.mapFilters.reason===x.reason_code?'selected':''}>${esc(x.reason_label_id)}</option>`).join('')}</select></div><div id="adminMap" class="admin-map"></div><div class="meta" style="margin-top:8px">Marker memakai GPS check-in VISIT. Garis menunjukkan urutan check-in, bukan actual road travelled. Pure BY WA tidak memiliki marker.</div></div>`}
function mapCallRows(){const data=filteredData(),intel=buildAdminIntelligence({...data,taxonomy:S.taxonomy,language:S.lang,minDirectionalSample:analysisRules().minDirectionalSample}),counts={};return data.calls.filter(c=>callMethod(c)==='VISIT'&&Number.isFinite(Number(c.checkin_latitude))&&Number.isFinite(Number(c.checkin_longitude))).filter(c=>!S.mapFilters.result||c.result===S.mapFilters.result).filter(c=>!S.mapFilters.reason||c.observed_reason_code===S.mapFilters.reason).sort((a,b)=>String(a.checkin_at||a.call_timestamp).localeCompare(String(b.checkin_at||b.call_timestamp))).map(c=>{counts[c.visit_id]=(counts[c.visit_id]||0)+1;return{c,i:counts[c.visit_id],intel}})}
function initAdminMap(){const el=document.getElementById('adminMap');if(!el)return;if(!window.L){el.innerHTML='<div class="empty">Map library gagal dimuat. Pastikan internet aktif.</div>';return}const rows=mapCallRows();if(!rows.length){el.innerHTML='<div class="empty">Tidak ada VISIT dengan GPS pada filter ini.</div>';return}const L=window.L,map=L.map(el),groups={};L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap contributors'}).addTo(map);const allLatLng=[];for(const {c,i,intel} of rows){const ll=[Number(c.checkin_latitude),Number(c.checkin_longitude)],v=S.visits.find(x=>x.id===c.visit_id)||{},attempts=intel.attemptsByCall[c.id]||[],recovered=intel.recoveredMap[c.id],photos=intel.photosByCall[c.id]||[],d=intel.detailMap[c.id]||{},stocks=intel.stockByCall[c.id]||[];allLatLng.push(ll);(groups[c.visit_id]??=[]).push(ll);const rich=compactReasonDetail(d,stocks);const popup=`<div class="map-popup"><b>#${i} · ${esc(c.outlet_id||'')} · ${esc(c.outlet_name)}</b><div>${esc(v.visit_date||'')} · ${esc(profileName(c.jovis_user_id))} · ${esc(v.salesman_name||'')}</div><div>${fmtTime(c.checkin_at)}–${fmtTime(c.checkout_at)} · ${fmtDuration(c.duration_seconds)}</div><div>${esc(c.route_status)} · <b>${esc(c.result)}</b>${c.result==='EC'?` · ${fmtCurrency(c.omzet)}`:''}</div>${c.result==='NON_EC'?`<div>Actual: ${esc(observedLabel(c,S.taxonomy,S.lang))}</div><div>SFA: ${esc(sfaLabel(c,S.taxonomy,S.lang)||'Unresolved')}</div>${rich?`<div>${esc(rich)}</div>`:''}<div>Recovery: ${recovered?`RECOVERED ${fmtCurrency(recovered.omzet)}`:(attempts.at(-1)?.outcome||'-')}</div><div>Evidence: ${esc(c.evidence||'-')}</div>`:''}<div>GPS ${Number(c.checkin_latitude).toFixed(5)}, ${Number(c.checkin_longitude).toFixed(5)} ±${Math.round(Number(c.checkin_accuracy_m)||0)}m</div>${photos.length?`<button class="map-photo-btn" data-photo-call="${c.id}">📷 ${photos.length} foto</button>`:''}</div>`;L.marker(ll).addTo(map).bindTooltip(`#${i}`,{permanent:true,direction:'top'}).bindPopup(popup)}for(const points of Object.values(groups))if(points.length>1)L.polyline(points).addTo(map);map.fitBounds(allLatLng,{padding:[24,24],maxZoom:16})}

function renderAdminAnalysis(){if(!isAdmin())return`<div class="card">Unauthorized</div>`;const data=filteredData(),intel=buildAdminIntelligence({...data,taxonomy:S.taxonomy,language:S.lang,minDirectionalSample:analysisRules().minDirectionalSample});return `<div class="card"><span class="pill blue">RULE-BASED RESEARCH</span><h1 style="margin-top:8px">Pertanyaan Join Visit</h1><p class="muted">Analysis hanya memakai field yang dicapture; sample kecil ditandai Evidence Insufficient.</p>${adminFilterBar()}</div>${intel.questions.map(q=>`<div class="card research-card"><div class="section-head"><div><span class="pill ${q.status==='DIRECTIONAL'?'good':'warn'}">${q.status==='DIRECTIONAL'?'Directional':'Evidence Insufficient'}</span><h2 style="margin-top:8px">${q.id} · ${esc(q.title)}</h2></div><b>N=${q.sample}</b></div><p>${esc(q.answer)}</p></div>`).join('')}<div class="card"><h2>Recovery Channel</h2><div class="kpis">${Object.entries(intel.channels).map(([k,x])=>`<div class="kpi"><b>${x.recovered}/${x.attempts}</b><span>${k} recovered · ${x.conversion}% · ${fmtCurrency(x.revenue)}</span></div>`).join('')}</div></div><div class="card"><h2>BMA · Pengakuan Salesman</h2><div class="kpis"><div class="kpi"><b>${intel.bombing.YES||0}</b><span>Order sebelumnya terlalu besar: Ya</span></div><div class="kpi"><b>${intel.bombing.NO||0}</b><span>Tidak</span></div><div class="kpi"><b>${intel.bombing.UNKNOWN||0}</b><span>Tidak tahu</span></div><div class="kpi"><b>${intel.bombing.UNANSWERED||0}</b><span>Belum dicapture</span></div></div><div class="notice" style="margin-top:10px">Claim salesman bukan bukti bombing. Validasi final harus menggunakan DBase historical order: last IPT/order value vs normal outlet, delivery lag, remaining stock, dan next EC.</div></div>`}

function renderCallTable(data,admin=false){return `<div class="card"><div class="section-head"><h2>Call Detail</h2><span class="small muted">${data.calls.length} rows</span></div><div class="table-wrap"><table class="table"><thead><tr><th>Time</th><th>Method</th><th>Kode Toko</th><th>Nama Toko</th><th>Result</th><th>Omzet</th><th>Actual</th><th>SFA</th><th>Recovery</th><th>GPS</th><th></th></tr></thead><tbody>${data.calls.map(c=>{const rec=recoveryAttemptsFor(c.id).find(x=>x.outcome==='RECOVERED_EC');return`<tr><td>${fmtTime(c.checkin_at||c.call_timestamp)}</td><td>${callMethod(c)}</td><td>${esc(c.outlet_id||'')}</td><td>${esc(c.outlet_name)}</td><td>${c.result}</td><td>${c.result==='EC'?fmtCurrency(c.omzet):''}</td><td>${c.result==='NON_EC'?esc(observedLabel(c,S.taxonomy,S.lang)):''}</td><td>${c.result==='NON_EC'?esc(sfaLabel(c,S.taxonomy,S.lang)||'Unresolved'):''}</td><td>${rec?`RECOVERED ${fmtCurrency(rec.omzet)}`:''}</td><td>${c.checkin_latitude!=null?`${Number(c.checkin_latitude).toFixed(5)}, ${Number(c.checkin_longitude).toFixed(5)}`:'-'}</td><td><button class="btn btn-secondary" data-edit-call="${c.id}" data-visit-id="${c.visit_id}">Edit</button>${c.result==='NON_EC'&&!admin?`<button class="btn btn-soft" data-recovery-call="${c.id}">Recovery</button>`:''}</td></tr>`}).join('')}</tbody></table></div></div>`}

function exactSfaOptions(selected=''){return S.taxonomy.filter(r=>r.active&&r.reason_type==='sfa').map(r=>`<option value="${r.reason_code}" ${selected===r.reason_code?'selected':''}>${esc(S.lang==='id'?r.reason_label_id:r.reason_label_en)}</option>`).join('')}
function renderSfaRecovery(data){if(!isAdmin())return'';const legacy=data.calls.filter(c=>c.result==='NON_EC'&&normalizedCaptureType(c)==='LEGACY'),unresolved=legacy.filter(c=>!exactSfaCode(c)&&!safeLegacyExactCode(c));return `<div class="card"><div class="section-head"><div><h2>SFA Legacy Recovery</h2><div class="meta">Nilai legacy dipreserve; exact E-Work ditambahkan ke Call ID yang sama.</div></div><span class="pill ${unresolved.length?'warn':'good'}">${unresolved.length} unresolved</span></div>${unresolved.length?`<div class="table-wrap"><table class="table"><thead><tr><th>Outlet</th><th>Actual</th><th>Legacy</th><th>Evidence</th><th>Suggestion</th><th>Exact</th><th></th></tr></thead><tbody>${unresolved.map(c=>{const sug=recoverySuggestion(c);return`<tr><td>${esc(c.outlet_id||'')}<br>${esc(c.outlet_name)}</td><td>${esc(observedLabel(c,S.taxonomy,S.lang))}</td><td>${esc(legacySfaLabel(c,S.taxonomy,S.lang)||c.sfa_reason_code||'-')}</td><td>${esc(c.evidence||'-')}</td><td>${sug?esc(taxonomyLabel(sug.code)):'-'}</td><td><select class="select" data-recovery-select="${c.id}"><option value="">Pilih</option>${exactSfaOptions(sug?.code||'')}</select></td><td><button class="btn btn-primary" data-confirm-recovery="${c.id}">Confirm</button></td></tr>`}).join('')}</tbody></table></div>`:`<div class="notice good">Semua legacy pada filter ini sudah resolved.</div>`}</div>`}
async function confirmLegacyRecovery(callId){const c=S.calls.find(x=>x.id===callId&&!x.is_deleted);if(!c)return;const code=document.querySelector(`[data-recovery-select="${callId}"]`)?.value||'';if(!code){showToast('Pilih exact reason');return}const patch={...c,sfa_reason_exact_code:code,sfa_capture_type:'LEGACY',sfa_recovery_status:'MANUAL_CONFIRMED',client_updated_at:nowISO()};patch.reason_match_status=dbCompatibleMatchStatus(patch);await Local.put('calls',patch);S.calls[S.calls.findIndex(x=>x.id===callId)]=patch;await enqueue('call','UPSERT',patch);render();showToast('Exact E-Work confirmed')}
...
async function runQaDiagnostics(){const checks=[];try{const x=await Cloud.diagnosticSnapshot();checks.push(['Supabase/RLS','PASS',`${x.visits.length} visit · ${x.calls.length} calls · ${x.reasonDetails.length} details`])}catch(e){checks.push(['Supabase/RLS','FAIL',e.message])}try{await Local.all('reasonDetails');checks.push(['IndexedDB v2','PASS','Rich evidence stores available'])}catch(e){checks.push(['IndexedDB v2','FAIL',e.message])}S.diagnostics=checks;await loadDiagnosticsData();render()}
function renderSyncDiagnostics(){return `<div class="card"><div class="section-head"><h2>Sync & Diagnostic</h2><button class="btn btn-secondary" data-action="runDiagnostics">Run Diagnostic</button></div>${S.diagnostics?S.diagnostics.map(([n,s,m])=>`<div class="diag-row"><span class="pill ${s==='PASS'?'good':'bad'}">${s}</span><div><b>${esc(n)}</b><div class="meta">${esc(m)}</div></div></div>`).join(''):''}${S.queueItems.length?`<div class="notice warn" style="margin-top:10px">Queue ${S.queueItems.length} item · ${S.queueItems.filter(x=>x.status==='ERROR').length} error</div><button class="btn btn-secondary" style="margin-top:8px" data-action="retryAllErrors">Retry All Errors</button>`:`<div class="notice good" style="margin-top:10px">Queue kosong.</div>`}</div>`}
...
async function handleClick(e){
  const b=e.target.closest('button,[data-route],[data-open-visit],[data-edit-visit],[data-edit-call],[data-confirm-recovery],[data-recovery-call],[data-call-method],[data-route-status],[data-result],[data-observed],[data-sfa],[data-remove-stock],[data-show-call],[data-admin-edit-call],[data-photo-call],[data-toggle-user]');if(!b)return;
